...
var magicedenCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if meCollection != "" {
			...
			nftMintAddresses, err := getMagicEdenNFTs(meCollection, meBearToken, rateLimit)
			if err != nil {
				fmt.Println("")
				fmt.Println("Error in attempting to discover NFTs in collection:", err.Error())
			} else {
				fmt.Println("")
				fmt.Println(len(nftMintAddresses))
				for s, _ := range nftMintAddresses {
					fmt.Println(s)
				}
			}
...
func getMagicEdenNFTs(meCollection string, meAuthToken string, rateLimit int) (map[string]string, error) {
